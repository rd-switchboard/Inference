...
public class Importer {
	...
	public Importer(Properties properties) throws JAXBException {
		...
		searchRequest = dynamoDB.getTable("SearchRequest");
		...
		linksFolder = new File(linksFolder + "/link");
		...
		for (File file : files) 
			if (!file.isDirectory()) {
				...
				for (String linkPath : result.getLinks()) {
					...
					if (searchResult.getItem(new PrimaryKey().addComponent("URL", link.getLink())) == null) {
