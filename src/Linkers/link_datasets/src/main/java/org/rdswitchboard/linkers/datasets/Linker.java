package org.rdswitchboard.linkers.datasets;
...
//	private PrintWriter logger; 
	...
	private Map<String, Set<Long>> datasets = new HashMap<String, Set<Long>>();
	 ...
	 */
	public void process() {
		loadDois("Dryad", "Dataset", "doi");
		loadDois("RDA", "Dataset", "identifier_doi");
	...
	private void loadDoi(String doi, Long nodeId) {
		...
		if (null != doi) {
			if (datasets.containsKey(doi))
				datasets.get(doi).add(nodeId);
			else
				datasets.put(doi, Sets.newHashSet(nodeId));
		}
	}
	
	private void createRelationships() {
		System.out.println("Processing " + datasets.size() + " unique DOI's"); 
		...
		try {
			for (Entry<String, Set<Long>> entry : datasets.entrySet()) {
				...
				if (nodeIds.size() > 1) {
					...
					for (int i1 = 0; i1 < ids.length - 1; ++i1) {
						...
						for (int i2 = i1 + 1; i2 < ids.length; ++i2) {
							if (Neo4jUtils.findRelationship(start, ids[i2], knownAs, Direction.BOTH) == null) {
